...
function useSave(lecountr, message, serverSaves, numb, thec, lastCounterId) {
	if (lecountr.saves >= 10) {
		message.react(useCustomEmoji ? customEmojiList.warn : '⚠️')
		...
		message.react(useCustomEmoji ? customEmojiList.ruin : '❌')
...
module.exports.eventLogic = async (message, client) => {
	...
	if (thec !== String(numb + 1)) {
		...
		if (message.content.length >= 1500) {
			message.react(useCustomEmoji ? customEmojiList.ignored : "⛔")
			if (longMessageEasterEgg) message.reply(longMessageEasterEggContent)
		} else {
			useSave(lecountr, message, serverSaves, numb, thec, lastCounterId)
		}
		lecountr.increment('wrongNumbers');
	...
	if (useCustomEmoji) {
		...
		switch (thec) {
			...
			default: message.react(highscore < thec ? "☑" : "✅"); break;
		}
	}
	numb++
