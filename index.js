...
client.on('messageDelete', async message => {
	if (message.channel.id !== countingCh && message.author.bot && message.attachments.size !== 0 && message.stickers.size !== 0 || (message.type !== "DEFAULT" || message.type !== "REPLY")) return

	let [lecountr] = await client.db.Counters.findOrCreate({ where: { userID: message.author.id } });
	if (lecountr.banned) return;

	if (validateExpression(message.content.split(" ")[0]) && message.content.toUpperCase() !== "INFINITY") {
